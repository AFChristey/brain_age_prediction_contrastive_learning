...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        # print('THIS IS THE X SHAPEc right before model')
...
class SupConResNet(nn.Module):
    ...
    def __init__(self, name='resnet50', head='mlp', feat_dim=128, num_sites=6, grl_layer=0, lambda_val=1.0, modality="OpenBHB"):
        super().__init__()
        model_fun, dim_in = model_dict[name]
        if modality == "OpenBHB":
            self.encoder = model_fun()
        else:
            self.encoder = model_fun(in_channels=2)
        ...
        self.classifier = SiteClassifier(feat_dim, num_sites, grl_layer, lambda_val)
